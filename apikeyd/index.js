...
function logCyber(message, color = 'cyan') {
  ...
  console.log(neon(`[${time}] ⚡️ ${message}`));
}
...
app.post('/api/save-user', (req, res) => {
  ...
  if (!firstname || !lastname || !email || !apiKey) {
    return res.status(400).json({ error: 'firstname, lastname, email and apiKey required' });
  }

  // INSERT user + langsung online + set last_seen
  const insertUser = `
    INSERT INTO users (firstname, lastname, email, is_online, last_seen)
    VALUES (?,?,?,?, datetime('now'))
  `;

  db.run(insertUser, [firstname, lastname, email, 1], function(err) {
    if (err) {
    ...
    db.run(insertKey, [userId, apiKey], function(err2) {
      ...
      db.get(`
        ...
        [userId],
        (e, row) => {
          if (e) return res.status(500).json({ error: 'DB fetch failed', details: e.message });
...
app.get('/api/validate', (req, res) => {
  ...
  db.get(`SELECT * FROM apikeys WHERE api_key = ?`, [key], (err, row) => {
    ...
    res.json({ valid: true, key: row.api_key, createdAt: row.created_at });
  });
});
